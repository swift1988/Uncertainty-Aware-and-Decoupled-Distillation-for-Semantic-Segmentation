...
from dataset.camvid import CamvidValSet, CamvidTrainSet
...
class Evaluator(object):
    def __init__(self, args, num_gpus):
        ...
        if args.dataset == 'citys':
            ...
            self.val_dataset = CSValSet(args.data, './dataset/list/cityscapes/val.lst', crop_size=(512, 1024))
            ...
            args.data = '/disk0/evannnnnnn/home/lee/data/VOCAug'
    ...
    def predict_whole_refine(self, net, image, tile_size, target):
        interp = nn.Upsample(size=tile_size, mode='bilinear', align_corners=True)
    
        prediction = net._forward(image.cuda(), target)
    ...
    def eval(self):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            if self.args.save_pred:
                root_path = 'work-dir/visualization_prediction/{}/{}'.format(args.dataset, os.path.splitext(filename[0])[0])

                if not os.path.exists(root_path):
                    os.makedirs(root_path)
                    utils.save_image(image[0][[2, 1, 0], :, :], os.path.join(root_path, 'image.png'), normalize=True)
                    img = get_color_pallete(target[0].cpu().numpy(), args.dataset)
                    img.save(os.path.join(root_path, 'color_gt.png'))
            ...
            with torch.no_grad():
                for scale in scales:
                    ...
                    # scaled_probs = self.predict_whole_refine(model, scale_image, tile_size, target)
            ...
            if self.args.save_pred:
                ...
                mask.save(os.path.join(root_path, '{}.png'.format(args.method)))
...
if __name__ == '__main__':
    ...
    # TODO: optim code
    # args.outdir = os.path.join(args.save_dir, 'prediction')
    args.outdir = args.save_dir
    if args.save_pred:
