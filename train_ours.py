...
pretrained = {
    'resnet18_v1s': 'pretrained/resnet18-imagenet.pth',
    'mobilenetv2' : 'pretrained/mobile.pth',
}
...
def parse_args():
    ...
    parser.add_argument('--mask-type', type=str, default='Ent')
    parser.add_argument('--threshold', type=float, default=0.5)
    parser.add_argument('--gamma', type=float, default=1.0)

    
    parser.add_argument("--lambda-logit", type=float, default=0., help="lambda_kd")
    parser.add_argument('--lambda-dmskd', type=float, default=0.)  
    parser.add_argument('--lambda-dmskd-intra', type=float, default=1.5)
    parser.add_argument('--lambda-dmskd-inter', type=float, default=1.0)
    ...
    args.student_pretrained_base = pretrained[args.student_backbone]
    # args.student_pretrained_base = 'None'
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.dataset == 'citys':
            args.data = '/disk0/evannnnnnn/home/lee/data/City'
            ...
            args.data = '/data/lee/home/Data/VOCAug'
            ...
            args.data = '/disk0/evannnnnnn/home/lee/data/ADEChallengeData2016'
            ...
            args.data = '/disk0/evannnnnnn/home/lee/data/CamVid'
        ...
        # create criterion
        self.criterion_dmskd = CriterionMaskDecoupleStructuralKD(args.ignore_label, self.num_class).to(self.device)
        self.criterion_kd = CriterionKD_ours(temperature=args.kd_temperature, mask_type=args.mask_type, gamma = args.gamma).to(self.device)
        ...
        if args.resume:
            if os.path.isfile(args.resume):
                ...
                self.s_model.load_state_dict(checkpoint["model"])
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            dmskd_loss = torch.tensor(0.).cuda()
            
            if args.lambda_logit != 0:            
                kd_loss = self.criterion_kd(s_outputs[0], t_outputs[0]) * self.args.lambda_logit
            
            if args.lambda_dmskd != 0:
                
                intra_skd, inter_skd = self.criterion_dmskd(s_outputs[-1], t_outputs[-1], targets)

                dmskd_loss = args.lambda_dmskd * (intra_skd * args.lambda_dmskd_intra + inter_skd * args.lambda_dmskd_inter)
                
            losses = task_loss + kd_loss + dmskd_loss
            ...
            dmskd_loss_reduced = self.reduce_mean_tensor(dmskd_loss)
            ...
            if iteration % log_per_iters == 0 and save_to_disk:
                logger.info(
                    ...
                    "|| DMSKD Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
                        iteration, args.max_iterations, self.optimizer.param_groups[0]['lr'], task_losses_reduced.item(),
                        kd_losses_reduced.item(), 
                        dmskd_loss_reduced.item(),
                        str(datetime.timedelta(seconds=int(time.time() - start_time))), eta_string))

            if iteration % save_per_iters == 0 and save_to_disk:
                save_checkpoint(self.s_model, self.optimizer, iteration, self.args, is_best=False)
        ...
        if (args.distributed is not True) or (args.distributed and args.local_rank == 0):
            save_checkpoint(self.s_model, self.optimizer, self.args.max_iterations, self.args, is_best=False)
    ...
    def validation(self, iteration):
        ...
        if (args.distributed is not True) or (args.distributed and args.local_rank == 0):
            save_checkpoint(self.s_model, self.optimizer, iteration, self.args, is_best)
        synchronize()


def save_checkpoint(model, optimizer, iterations, args, is_best=False):
    """Save Checkpoint"""
    directory = os.path.expanduser(args.save_dir)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = 'regular.pth'
...
if __name__ == '__main__':
    ...
    if args.distributed:
        ...
        synchronize()
    logger = setup_logger("semantic_segmentation", args.save_dir)
