...
class DeepLabV3(SegBaseModel):
    ...
    def _forward(self, x, target):
        ...
        labels = target.unsqueeze(1).float().clone()
        labels = torch.nn.functional.interpolate(labels, (x.shape[2], x.shape[3]), mode='nearest')
        labels = labels.squeeze(1).squeeze(0).long()
        ...
        for i in labels:
            idxs = (labels == i).float()
            matrix.append(idxs / idxs.sum())
        matrix = torch.stack(matrix, dim=1)
        # self-affinity
        # matrix = torch.mm(c4.T, c4)
        # matrix = torch.softmax(matrix, dim=-1)
        # print(matrix.shape)
        # non-local
        
        c4_refine = torch.mm(c4, matrix).view(c, h, w).unsqueeze(0)

        x, x_feat_after_aspp = self.head(c4_refine)
...
def get_deeplabv3(backbone='resnet18', local_rank=None, pretrained=None, 
    ...
    if pretrained != 'None':
        if local_rank is not None:
            ...
            if backbone == 'resnet101':
                ...
                model.load_state_dict(torch.load(pretrained, map_location=device)['model'])
            
            # state_dict = torch.load(pretrained, map_location=device)['model']
...
if __name__ == '__main__':
    model = get_deeplabv3()
    img = torch.randn(1, 3, 360, 360)
    output = model._forward(img)
    for o in output:
        print(o.shape)
